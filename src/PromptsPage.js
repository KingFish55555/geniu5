...
import { FileText, Plus, Save, Trash2, ChevronDown, Settings, Edit2, FileInput, FileOutput, GripVertical } from 'lucide-react';
import ModuleEditorModal from './ModuleEditorModal';

const PromptsPage = ({ 
  prompts, currentPrompt, setCurrentPrompt, 
  savePrompt, deletePrompt, restoreDefaultPrompts, 
  onOpenSwitcher, onAddModule, onDeleteModule, onModuleOrderChange 
}) => {
  ...
  const handleUpdateModule = (updatedModule) => {
    ...
    onModuleOrderChange(newModules);
  ...
  const handleDragEnd = () => {
    ...
    const modules = [...(currentPrompt.modules || [])];
    const [movedItem] = modules.splice(draggedItem, 1);
    modules.splice(dragOverItem, 0, movedItem);
    onModuleOrderChange(modules);
    setDraggedItem(null);
    setDragOverItem(null);
  };
  ...
  const handleTouchMove = (e) => {
    ...
    let newDragOverIndex = dragOverItem;
    ...
    if (newDragOverIndex !== dragOverItem) {
        setDragOverItem(newDragOverIndex);
    }
  };
      ...
      <div className="page-content prompts-page-container">
        <div className="content-area prompts-page-area">
          ...
          {currentPrompt ? (
            <div className="setting-card">
              <div className="card-header">
                  <div className="card-title">{currentPrompt.name} 模組列表</div>
              ...
              <div 
                className="card-content module-list" 
                ref={listRef}
                onDragEnd={handleDragEnd}
                onTouchEnd={handleDragEnd}
                onTouchMove={handleTouchMove}
              >
                {(currentPrompt.modules || []).map((module, index) => (
                  <div 
                    key={module.id} 
                    className={`
                      module-list-item
                  ...
                  >
                    ...
                    <button
                      className="edit-module-btn delete"
                      onClick={() => onDeleteModule && onDeleteModule(module.id)}
                    >
                        <Trash2 size={16}/>
                    </button>
                  </div>
